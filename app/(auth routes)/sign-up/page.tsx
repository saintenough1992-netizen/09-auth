...
export default function SignUp() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const user = await register({ email: String(formData.get('email')), password: String(formData.get('password')) });
  ...
  return <main className={css.mainContent}><h1 className={css.formTitle}>Sign up</h1><form className={css.form} onSubmit={handleSubmit}>
    ...
    <div className={css.actions}><button type="submit" className={css.submitButton}>Register</button></div>
